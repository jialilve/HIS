...
public class DoctorList extends JFrame {
	...
	public DoctorList() {
		...
		editButton.setFont(new Font("华文楷体", Font.PLAIN, 17));
		editButton.setBounds(636, 353, 93, 23);
		...
		deleteButton.setBounds(636, 399, 93, 23);
	...
	protected void EditButton(ActionEvent e) {
		...
		doctor.setId(dtm.getValueAt(this.DoctorList.getSelectedRow(), 0).toString());
		doctor.setName(dtm.getValueAt(this.DoctorList.getSelectedRow(), 1).toString());
		doctor.setSex(dtm.getValueAt(this.DoctorList.getSelectedRow(), 2).toString());
		doctor.setAge((int) dtm.getValueAt(this.DoctorList.getSelectedRow(), 3));
		doctor.setLevel(dtm.getValueAt(this.DoctorList.getSelectedRow(), 4).toString());
		doctor.setTel(dtm.getValueAt(this.DoctorList.getSelectedRow(), 5).toString());
		doctor.setDeparment(dtm.getValueAt(this.DoctorList.getSelectedRow(), 6).toString());
		
		if(Indexframe.adddoctor==null)
		{
			Indexframe.adddoctor=new AddDoctor();
		}
		Indexframe.adddoctor.editDoctor(doctor);
		Indexframe.adddoctor.setVisible(true);
		queryAlldoctor();
	}
	protected void deletebutton(ActionEvent e) {
		// TODO Auto-generated method stub
		if(JOptionPane.showConfirmDialog(this, "是否删除此医师？","正在删除",JOptionPane.YES_NO_OPTION)==JOptionPane.OK_OPTION)
		{
			String id=dtm.getValueAt(this.DoctorList.getSelectedRow(), 0).toString();
			DoctorDao doctordao=new DoctorDao();
			JOptionPane.showConfirmDialog(this, doctordao.DeleteDoctor(id));
			queryAlldoctor();
		}
	}
