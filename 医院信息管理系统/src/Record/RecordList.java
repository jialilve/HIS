...
public class RecordList extends JFrame {
	 ...
	 * Create the frame.
	 */
	public RecordList(String id,int lid) 
	{
		this.id=id;
		this.lid=lid;
	...
	protected void editRecord(ActionEvent e) 
	{
		// TODO Auto-generated method stub
		String Pid=dtm.getValueAt(this.table.getSelectedRow(), 0).toString();
		String Doid=dtm.getValueAt(this.table.getSelectedRow(), 2).toString();
		String Date=dtm.getValueAt(this.table.getSelectedRow(), 5).toString();
		String disease=dtm.getValueAt(this.table.getSelectedRow(), 4).toString();
		EditRecord ed=new EditRecord(Doid,Pid,Date,disease);
		ed.setVisible(true);
	}
	...
	public void Querysome(ActionEvent e)
	{
		...
		if(this.lid==1)
		{
		    idd=this.Pid.getText();
		}
		else if(this.lid==2)
		{
			idd=this.Doid.getText();
		}
		System.out.println(this.Pid.getText()+idd);
		RecordDao patientdao=new RecordDao();
		List<Record> allList=patientdao.querysomerecord(this.id,idd,this.lid);
		for(Record st:allList)
		{
			Vector v=new Vector();
 			v.add(st.getPid());
